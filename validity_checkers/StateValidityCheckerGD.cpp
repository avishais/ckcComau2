...
State StateValidityChecker::sample_q() {
	...
	while (1) {
		...
		if ( withObs && (collision_state(q) || !check_angle_limits(q) || !checkEE(q)) )  {
			sampling_counter[1]++;
			continue;
		}
...
bool StateValidityChecker::IKproject(State &q, bool includeObs) {
	...
	if (includeObs && withObs && (collision_state(q) || !checkEE(q)) )
		return false;
...
bool StateValidityChecker::checkEE(State q) {

	State q1(6), q2(6);
	seperate_Vector(q, q1, q2);

	FK6(q1);
	Matrix T1 = get_FK_solution();

	FK6(q2);
	Matrix T2 = get_FK_solution();
...
bool StateValidityChecker::isValidRBS(State &q) {
	...
	if ( withObs && (collision_state(q) || !checkEE(q)) )
		return false;
